...
sudo chroot ./alp/root/ /bin/sh -c "apk update && apk upgrade && apk add pkgconfig git build-base sdl3-dev sdl3_ttf-dev sdl3_image-dev make cmake libarchive harfbuzz fmt fmt-dev libxml2 libxml2-dev inih spdlog-dev spdlog && git clone https://github.com/complexlogic/big-launcher.git && cd big-launcher && mkdir build && cd build && mkdir /app && cmake .. -DCMAKE_INSTALL_PREFIX=/app && make && make --install && exit"
